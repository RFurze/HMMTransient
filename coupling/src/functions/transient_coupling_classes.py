...
class MetaModel3:
    ...
    #     """
    #     xi is a list of 9 arrays:
    #       xi = [
    #         h, p,
    #         lmbx, lmby, lmbz,
    #         gradpx, gradpy, gradpz,
    #         f   (the cavitation function)
    #       ]
    ...
    #     H, P, U, V, lmbZ, dPdx, dPdy, gradPz, dFdx, dFdy, dFdz, F, Hdot, Pdot = xi
    ...
    #         [Hn, Pn, Un, Vn, dPdxn, dPdyn, dFdxn, dFdyn, Fn, Hdotn, Pdotn]
    #     )

    #     # We'll keep track of newly accepted points in "new_unscaled" with shape (#,6).
    #     new_unscaled = np.zeros((0, 9))
    ...
    #         chosen_unscaled[:, 9] = unscale(chosen_centers[:, 9], Hdot_min, Hdot_max)
    #         chosen_unscaled[:, 10] = unscale(chosen_centers[:, 10], Pdot_min, Pdot_max)
    ...
    #         existing_array[12, :] = chosen_unscaled[:, 9]  # Hdot
    #         existing_array[13, :] = chosen_unscaled[:, 10]  # Pdot

    #         self.existing_xi_d = existing_array
    #         new_unscaled = chosen_unscaled  # shape (M,11)
    ...
    #         Hdotx = scale_column(ex[12, :], Hdot_min, Hdot_max)
    #         Pdotx = scale_column(ex[13, :], Pdot_min, Pdot_max)
    #         existing_norm = np.column_stack(
    #             [Hx, Px, Ux, Vx, dx, dy, dFdx, dFdy, Fx, Hdotx, Pdotx]
    #         )
    ...
    #             new_unscaled[:, 9] = (
    #                 accepted_norm[:, 9] * (Hdot_max - Hdot_min) + Hdot_min
    #             )
    #             new_unscaled[:, 10] = (
    #                 accepted_norm[:, 10] * (Pdot_max - Pdot_min) + Pdot_min
    #             )

    #             # Build new_points => shape (9, #acc)
    #             # fill the same rows
    #             n_acc = new_unscaled.shape[0]
    #             new_points = np.zeros((14, n_acc))
    ...
    #             new_points[12, :] = new_unscaled[:, 9]  # Hdot
    #             new_points[13, :] = new_unscaled[:, 10]  # Pdot
    ...
    #         Hdotx = scale_column(ex_updated[12, :], Hdot_min, Hdot_max)
    #         Pdotx = scale_column(ex_updated[13, :], Pdot_min, Pdot_max)
    ...
    #         xi_d = [np.array([]) for _ in range(14)]
    #     else:
    #         Hd = new_unscaled[:, 0]
    #         Pd = new_unscaled[:, 1]
    #         Ud = new_unscaled[:, 2]
    ...
    #         Fd = new_unscaled[:, 8]
    #         Hdotd = new_unscaled[:, 9]
    #         Pdotd = new_unscaled[:, 10]
