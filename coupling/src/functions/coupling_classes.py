import numpy as np
from coupling.src.functions.coupling_helper_fns import (
    build_task_list,
    build_task_list_transient,
)
...
from CONFIGPenalty import ND_FACTOR, RO_THETA
...
class MetaModel3:
    ...
    def build(self, xi, order, init, theta=None):
        ...
        # ro_theta = RO_THETA
        # r0 = (-1.0 / ro_theta) * math.log(0.5)

        # tree = cKDTree(new_data_norm)
        # # query k=2 because the closest point to each is itself (distance 0)
        # dists, idxs = tree.query(new_data_norm, k=2)
        # # take the second column → true nearest neighbour distance
        # nn_dists = dists[:, 1]
        ...
        # plt.tight_layout()
        # plt.savefig("SS_nn_distance_histogram.png")
        # plt.close()
        # logging.info(
        #     f"Saved nearest‐neighbour distance histogram to nn_distance_histogram.png"
        # )
        ...
        #     ex = self.existing_xi_d  # shape (9, old_count)
        ...
        #         dist_i = weighted_distance(
        #             existing_norm, new_data_norm[i], weight_f=2.0
        #         )
        #         if np.all(dist_i > r0):
        ...
        #     xi_d = [np.array([]) for _ in range(12)]
        # else:
        #     Hd = new_unscaled[:, 0]
        #     Pd = new_unscaled[:, 1]
        #     Ud = new_unscaled[:, 2]
        ...
        # tasks = build_task_list(xi_d)
        tasks = build_task_list(xi)  # 1:1 tasks
        xi_d = xi.copy()  # xi_d is the same as xi in this case
        return tasks, xi_d

    def build_transient(self, xi, order, init, theta=None):
